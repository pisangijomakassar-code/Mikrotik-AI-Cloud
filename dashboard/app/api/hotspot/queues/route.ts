...
export async function GET(request: NextRequest) {
  ...
  if (!user?.telegramId) return Response.json({ queues: [] })
  ...
  try {
    const res = await fetch(`${agentUrl}/queues/${user.telegramId}${qs}`, {
    ...
    return Response.json({ error: "Failed to fetch queues" }, { status: 502 })
  }
}
