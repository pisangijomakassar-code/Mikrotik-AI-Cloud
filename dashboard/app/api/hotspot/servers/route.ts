...
export async function GET(request: NextRequest) {
  ...
  if (!user?.telegramId) return Response.json({ servers: [] })
  ...
  try {
    const res = await fetch(
      `${agentUrl}/hotspot-servers/${user.telegramId}${qs}`,
    ...
    return Response.json({ error: "Failed to fetch hotspot servers" }, { status: 502 })
  }
}
