  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        ...
        >
          <Plus className="h-4 w-4" /> Tambah Profile
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-lowest/80">
                ...
                <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-border/20">Session Timeout</th>
                <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-border/20 hidden md:table-cell">Idle Timeout</th>
              ...
              {isLoading ? (
                ...
                profiles.map((profile, i) => (
                  <tr key={profile.name} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 py-2 text-sm text-slate-400 font-mono-tech">{profile.sessionTimeout || "--"}</td>
                    <td className="px-4 py-2 text-sm text-slate-400 font-mono-tech hidden md:table-cell">{profile.idleTimeout || "--"}</td>
          ...
          <div className="bg-surface-low border border-border/20 rounded-3xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-border/20">
              <h3 className="text-xl font-headline font-bold text-foreground">
                {editTarget ? `Edit: ${editTarget.name}` : "Tambah Profile"}
              </h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className={lbl}>Session Timeout</label>
                  <Input className={inp + " font-mono-tech"} placeholder="1h, 1d, 00:00:00" value={form.sessionTimeout} onChange={(e) => setForm((f) => ({ ...f, sessionTimeout: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <label className={lbl}>Idle Timeout</label>
                  <Input className={inp + " font-mono-tech"} placeholder="00:03:00" value={form.idleTimeout} onChange={(e) => setForm((f) => ({ ...f, idleTimeout: e.target.value }))} />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className={lbl}>Address Pool</label>
                <Input className={inp} placeholder="hs-pool-1" value={form.addressPool} onChange={(e) => setForm((f) => ({ ...f, addressPool: e.target.value }))} />
              </div>
            </div>
