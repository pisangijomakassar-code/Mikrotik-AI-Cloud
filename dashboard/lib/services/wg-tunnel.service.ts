...
export function generateWireguardScript(params: {
  ...
  return [
    ...
    "# Paste each line ONE AT A TIME into RouterOS terminal (New Terminal in Winbox)",
    "# Or paste all lines at once — each command is a complete single line.",
    "",
    "# 1. Create WireGuard interface",
    `/interface/wireguard/add name=wg-saas private-key="${clientPrivKey}" listen-port=13231 comment="MikroTik AI Agent"`,
    "",
    "# 2. Assign VPN IP to the interface",
    `/ip/address/add address=${vpnIp}/24 interface=wg-saas`,
    "",
    "# 3. Add server peer (entire command must be on one line)",
    `/interface/wireguard/peers/add interface=wg-saas public-key="${serverPubKey}" endpoint-address=${host} endpoint-port=51820 allowed-address=10.8.0.0/8 persistent-keepalive=25 comment="MikroTik AI Agent VPS"`,
    "",
    "# 4. Route management traffic over the tunnel",
    "/ip/route/add dst-address=10.8.0.0/8 gateway=wg-saas",
    "",
    "# 5. Masquerade outbound VPN traffic",
    `/ip/firewall/nat/add chain=srcnat out-interface=wg-saas action=masquerade comment="WG-SAAS NAT"`,
    "",
    ...
    "",
    "# Verify the tunnel (peer should show 'last-handshake' after ~30 seconds):",
