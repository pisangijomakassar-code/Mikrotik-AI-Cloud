...
export function generateOvpnScript(params: {
  ...
  return [
    ...
    "",
    "# 1. Import the CA certificate (upload ca.crt to router files first)",
    "# /certificate import file-name=ca.crt",
    "",
    "# 2. Create the OVPN client interface",
    ...
    "",
    "# 3. Add VPN IP address",
    "/ip address",
    `  add address=${vpnIp}/24 interface=ovpn-saas`,
    "",
    "# 4. Route management traffic over the tunnel",
    "/ip route",
    `  add dst-address=10.9.0.0/16 gateway=ovpn-saas`,
    "",
    "# 5. Masquerade outbound VPN traffic",
    "/ip firewall nat",
    `  add chain=srcnat out-interface=ovpn-saas action=masquerade comment="OVPN-SAAS NAT"`,
    "",
