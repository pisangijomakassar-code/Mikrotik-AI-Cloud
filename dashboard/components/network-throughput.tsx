    ...
    >
      ...
      {isLoading ? (
        <div className="space-y-4 animate-pulse">
          <div className="h-8 bg-[#222a3d] rounded-lg w-1/3" />
          <div className="h-4 bg-[#222a3d] rounded-lg w-1/2" />
          <div className="grid grid-cols-3 gap-4 mt-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-16 bg-[#222a3d] rounded-lg" />
            ))}
          </div>
        </div>
        ...
        <div className="h-32 flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-[10px] text-slate-600">Add a router to see live traffic</p>
        </div>
      ) : (
        <>
          {/* Summary stats */}
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-[#131b2e] rounded-lg p-4 border border-white/5">
              <div className="flex items-center gap-2 mb-2">
                <ArrowUp className="h-3.5 w-3.5 text-[#4ae176]" />
                <span className="text-[10px] text-slate-500 uppercase font-bold">Upload</span>
              </div>
              <span className="text-lg font-bold text-[#dae2fd] font-mono">{formatBytes(totalTx)}</span>
            </div>
            <div className="bg-[#131b2e] rounded-lg p-4 border border-white/5">
              <div className="flex items-center gap-2 mb-2">
                <ArrowDown className="h-3.5 w-3.5 text-[#4cd7f6]" />
                <span className="text-[10px] text-slate-500 uppercase font-bold">Download</span>
              </div>
              <span className="text-lg font-bold text-[#dae2fd] font-mono">{formatBytes(totalRx)}</span>
            </div>
            <div className="bg-[#131b2e] rounded-lg p-4 border border-white/5">
              <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-lg font-bold text-[#dae2fd] font-mono">{formatBytes(totalTx + totalRx)}</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {data.interfaces.slice(0, 6).map((iface) => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-mono text-[#4cd7f6] w-28 truncate">{iface.name}</span>
                  <div className="flex-1 h-1.5 bg-[#222a3d] rounded-full overflow-hidden flex">
                    <div
                      className="h-full bg-[#4ae176] rounded-l-full"
                      style={{ width: `${txPct}%` }}
                    />
                    <div
                      className="h-full bg-[#4cd7f6] rounded-r-full"
                      style={{ width: `${100 - txPct}%` }}
                    />
                  </div>
                  <div className="flex items-center gap-3 text-[10px] text-slate-400 w-40 justify-end">
                    <span>
                      <ArrowUp className="h-2.5 w-2.5 inline text-[#4ae176]" /> {formatBytes(iface.txBytes)}
                    </span>
                    <span>
                      <ArrowDown className="h-2.5 w-2.5 inline text-[#4cd7f6]" /> {formatBytes(iface.rxBytes)}
                    </span>
                  </div>
                </div>
              )
