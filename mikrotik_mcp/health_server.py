...
_MIKBOTAM_HEADER_RE = re.compile(r':put\s*\(\s*",(?P<lock>enable|disable),(?P<validity>[^,"]+),"\s*\)\s*;')


def _parse_mikbotam_on_login(on_login: str) -> dict:
    """Extract Mikbotam-style metadata from an on-login script.

    Returns dict with `validity` (e.g. "12h" or "") and `lockUser` (bool).
    Returns empty values when the script wasn't generated by Mikbotam pattern
    or cannot be parsed.
    """
    if not on_login:
        return {"validity": "", "lockUser": False}
    m = _MIKBOTAM_HEADER_RE.search(on_login)
    if not m:
        return {"validity": "", "lockUser": False}
    return {
...
def _build_mikbotam_on_login(validity: str, lock_user: bool) -> str:
    """Build Mikbotam-style on-login script for calendar-based voucher expiry.

    On first login the script creates a /system scheduler named after the user.
    The scheduler fires once after `validity` of *calendar* time (not connection
    time) and removes the user, the active session, and the hotspot cookie.

    Args:
        validity: RouterOS time string, e.g. "12h", "1d", "7d", "30d".
        lock_user: when True, also bind the user account to the device's MAC
            so the voucher cannot be reused on a different SSID/AP context.

    Reference: github.com/mikbotam/Mikbotam — hotspot/add_profile.php
    """
